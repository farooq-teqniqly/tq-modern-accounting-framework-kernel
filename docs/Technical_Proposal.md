...
**ID Generation**: All entity IDs use `Guid` directly, generated via `Guid.CreateVersion7()`. UUID version 7 provides time-ordered GUIDs that improve database indexing performance and are naturally sortable by creation time. Factory methods (e.g., `Account.Create()`) ensure consistent ID generation.
...
- Generated via factory methods (e.g., `Account.Create()`) to ensure consistent ID generation
